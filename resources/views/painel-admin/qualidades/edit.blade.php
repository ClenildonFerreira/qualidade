...
@section('title', 'Editar Produção Diária')
@section('content')
<h6 class="mb-4"><i>EDIÇÃO DA PRODUÇÃO DIÁRIA</i></h6><hr>
<form method="POST" action="{{route('qualidade.editar', $item)}}">
        @csrf
        @method('put')
...
<div class="col-md-3">
    <div class="form-group">
        ...
        <input value="{{$item->referencia}}" type="text" class="form-control" id="referencia" name="referencia" required>
    </div>
</div>
...
use App\Models\Revisora;
            ...
            $instrutor = Revisora::where('id', '=', $item->instrutor)->first();
            if($item->instrutor != '0'){
